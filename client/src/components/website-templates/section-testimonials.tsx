...
export default function SectionTestimonials({ section }: { section: SectionContent }) {
  ...
  return (
    <section className="py-24 sm:py-32 px-4 sm:px-6 relative overflow-hidden">
      ...
      {personality.showFloatingOrbs && <FloatingOrb color="var(--brand-primary, hsl(var(--primary)))" size={500} x="85%" y="25%" opacity={0.06} blur={120} />}
      {personality.showFloatingOrbs && <FloatingOrb color="var(--brand-accent, hsl(var(--primary)))" size={400} x="5%" y="75%" delay={4} opacity={0.05} blur={100} />}
          ...
          >
            {(() => {
              ...
              return personality.buttonStyle === "gradient-border" ? (
                <AnimatedGradientBorder borderWidth={2} borderRadius={24}>{featuredContent}</AnimatedGradientBorder>
              ) : featuredContent;
            })()}
          </motion.div>
        )}
            ...
            >
              {(() => {
                ...
                return personality.showHoverTilt ? (
                  <HoverTilt maxTilt={3} scale={1.01}>{testimonialCard}</HoverTilt>
                ) : testimonialCard;
              })()}
            </motion.div>
          ))}
