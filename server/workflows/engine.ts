...
/**
 * Launch Workflow - run the core generation flows in sequence
 */
export async function runLaunchWorkflow(ctx: WorkflowContext): Promise<void> {
  const launchSteps = [
    { type: "naming-domain", name: "Naming & Domain" },
    { type: "brand-kit", name: "Brand Kit" },
    { type: "website-plan", name: "Website Plan" },
    { type: "graphics", name: "Graphics" },
  ];

  const totalSteps = launchSteps.length;

  try {
    for (let index = 0; index < launchSteps.length; index++) {
      ...
      await storage.updateWorkflowJob(ctx.jobId, {
        status: "running",
        progress,
        result: { currentStep: step.type },
      });

      await storage.createActivityLog({
        projectId: ctx.projectId,
        action: `Launch: ${step.name}`,
        status: "running",
      });

      const runner = getBaseWorkflowRunner(step.type);
      if (!runner) {
        throw new Error(`Unknown workflow type: ${step.type}`);
      }

      const childJob = await storage.createWorkflowJob({
        projectId: ctx.projectId,
      ...
      await runner({ projectId: ctx.projectId, project: ctx.project, jobId: childJob.id });
    }
...
export function getWorkflowRunner(
  ...
  const runners: Record<string, (ctx: WorkflowContext) => Promise<void>> = {
    ...
    "launch": runLaunchWorkflow,
  };
  
  return runners[workflowType];
}

function getBaseWorkflowRunner(workflowType: string): ((ctx: WorkflowContext) => Promise<void>) | undefined {
  const runners: Record<string, (ctx: WorkflowContext) => Promise<void>> = {
    "naming-domain": runNamingDomainWorkflow,
    "brand-kit": runBrandKitWorkflow,
    "website-plan": runWebsitePlanWorkflow,
